...
async def find_similar_functions(task_description, threshold=0.7):
    ...
    distances, indices = index.search(task_embedding, k=1)  # Adjust k as needed
      ...
      "schema": {
        ...
        "properties": {
          "Understanding the Task": {
            "type": "string",
            "description": "This is a clear description of the task only. State what the task is and what dependencies."
          },
          "Implementation (Code)": {
            "type": "object",
     ...
     response = await client.post(
        ...
        json={
            ...
            "messages": [ {
                        ...
                        {
                        'role': 'user',
                        'content': f"<USER TASK><TASK DESCRIPTION>{task_description} <Data in file>{sample_file_data}.<Data in file> Use this data to help under how to to file related tasks<TASK DESCRIPTION><USER TASK><CONTEXT><PREVIOUS TASK>{task_context_description}<PREVIOUS TASK><PREVIOUS CODE>{task_context_code}<PREVIOUS CODE><CONTEXT>",
                        }],
...
def execute_code(is_python: bool):
  if is_python:
    ...
    return "BASH SCRIPT A WORK IN PROGRESS"
...
def sample_lines(path, sample_size_per_file=3, num_files_to_sample=5):
    ...
    return f"<DATA FROM FOLDERS>{'\n'.join(all_sampled_lines)}<DATA FROM FOLDERS>"
...
async def run_task(task: str = Query(..., description="Plain-English task description")):
    ...
    if file_content_path:
      ...
      if is_supported_file(file_path):
        sample_file_data = sample_lines(read_file_path__,50)
